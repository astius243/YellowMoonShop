...
public class SearchOrderController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = "TrackOrderController";
        try {
            String search = request.getParameter("txtSearchID");
            HttpSession session = request.getSession();
            Account acc = (Account) session.getAttribute("USER");
            if(search.length()>0){
                OrderBLO blo = new OrderBLO();
                Orders order = blo.getOrder(Integer.parseInt(search), acc.getUsername());
